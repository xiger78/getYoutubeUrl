---
title: Python3로 만든 유튜브 노래·뮤직비디오 검색·재생·MP3/MV 저장 GUI (Raspberry Pi / Windows)
tags:
  - Python3
  - tkinter
  - yt-dlp
...
Raspberry Pi(Debian 13 Trixie, arm64) 및 **Windows** 에서 Python3와 tkinter로 **유튜브 노래 검색·재생 리스트·스트리밍 재생·가사 표시·MP3 저장·MV 팝업 재생·MV MP4 저장**까지 한 창에서 처리하는 GUI 프로그램 **getYoutubeUrl** 을 만든 과정과 사용법을 정리합니다. **노래 검색**과 **뮤직비디오 검색**을 구분할 수 있고, MV는 **800×600 팝업**에서 **Full HD(1080p)** 로 재생·저장합니다. 유튜브 API 키는 필요 없으며, 검색·다운로드는 `yt-dlp`, 재생은 `libVLC(python-vlc)` 를 사용합니다.
...
|------|------|
| 프로그램명 | getYoutubeUrl |
| 목적 | 유튜브 검색(노래/MV) → 재생 리스트 → 재생·가사·MP3·MV 팝업·MV 저장 |
| MV 팝업 | 초기 800×600, Full HD(1080p), F11 전체화면 |
| MV 저장 | 재생 리스트 MV → MP4(1080p) 일괄·선택 저장 |
| GUI | tkinter (`python3-tk`) |
| 검색·다운로드 | `yt-dlp` |
| 재생 | `python-vlc` + 시스템 libVLC |
| 가사 | `syncedlyrics` |
| MP3·MV 변환 | 시스템 `ffmpeg` |
| GitHub | https://github.com/xiger78/getYoutubeUrl |
| 프로젝트 경로 (Pi) | `/home/pi/dev/getYoutubeUrl` |
...
- **MV MP4(1080p)** 재생 리스트 일괄·선택 저장
- **랜덤 재생**, 자동 다음 곡
- **Linux / macOS / Windows** 설치·실행 스크립트
...
- **왼쪽:** 검색 · 검색 결과 · 재생 리스트 · MP3/MV 다운로드 · 재생 컨트롤
- **오른쪽:** 가사 패널 (320px)
- **맨 아래:** 상태줄 (진행·오류 메시지)
...
| **⬇ 선택 MV 저장** | 선택 MV만 MP4(1080p) 저장 |
| **⬇ MV 저장 (전체)** | 리스트 MV 전체 MP4 저장 |
...
- 🎬 뮤직비디오 검색 → **추가 ↓** 로 리스트에 MV 추가
- **⬇ 선택 MV 저장** / **⬇ MV 저장 (전체)** 로 MP4(1080p) 저장
- 팝업 재생과 동일한 `best[height<=1080]` 포맷, **ffmpeg** 필요
...
| MP3 저장 실패 | `sudo apt install ffmpeg` (Windows: `setup-windows-manual.bat`) |
...
- `yt-dlp` 는 유튜브 변경에 맞춰 주기적으로 업데이트하는 것을 권장합니다.

## 14. 마무리

`getYoutubeUrl` 은 API 키 없이 유튜브를 검색(노래/MV)하고, 재생 리스트로 곡을 모은 뒤 오디오 재생·MV 팝업·가사·MP3·MV MP4 저장까지 처리하는 데스크톱 도구입니다. Raspberry Pi뿐 아니라 **Windows** 에서도 설치 스크립트로 바로 사용할 수 있습니다.
