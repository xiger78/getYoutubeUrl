...
- 検索結果 **ダブルクリックで即再生**（曲·MV、再生リストには追加しない）
- 検索結果を再生リストに **無制限に蓄積**（**追加 ↓** ボタン、URL 重複防止）
...
| ボタン / 操作 | 機能 |
|---------------|------|
| **追加 ↓** | 再生リストに追加（URL 重複はスキップ）。**リスト追加はこのボタンのみ** |
| **🎬 MV再生** | 選択 MV をポップアップで再生 |
| **解像度** | MV 再生·保存の最大解像度（HD / FHD / QHD / 2K / 4K） |
| **ブラウザで開く** | 既定ブラウザで YouTube を開く |
| **言語** | UI 言語切替 |
| **ダブルクリック** | 選択項目を **即再生**（曲·MV）。再生リストには追加しない |
...
- **ダブルクリック**: 再生リストに入れず即再生（曲はメイン、MV はポップアップ）
- **追加 ↓**: 再生リストに追加のみ（再生しない）
...
| v16 | 検索結果ダブルクリックで即再生、リスト追加は **追加 ↓** ボタン |
