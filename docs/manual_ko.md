...
- 검색 결과 **더블클릭으로 즉시 재생** (노래·MV, 재생 리스트 미추가)
- 검색 결과를 재생 리스트에 **무제한 누적** (`追加 ↓` 버튼, URL 중복 방지)
...
| 버튼 / 조작 | 기능 |
|-------------|------|
| **추가 ↓** | 재생 리스트에 추가 (URL 중복은 건너뜀). **리스트 추가는 이 버튼만 사용** |
| **🎬 MV 재생** | 선택 MV를 팝업에서 재생 |
| **해상도** | MV 재생·저장 최대 해상도 (HD / FHD / QHD / 2K / 4K) |
| **브라우저로 열기** | 기본 브라우저에서 YouTube 열기 |
| **언어** | UI 언어 전환 |
| **더블클릭** | 선택 항목 **즉시 재생** (노래·MV). 재생 리스트에는 추가되지 않음 |
...
- **더블클릭**: 재생 리스트에 넣지 않고 바로 재생 (노래는 메인 창, MV는 팝업)
- **추가 ↓**: 재생 리스트에만 추가 (재생하지 않음)
...
| v16 | 검색 결과 더블클릭 → 즉시 재생, 재생 리스트 추가는 **추가 ↓** 버튼 |
